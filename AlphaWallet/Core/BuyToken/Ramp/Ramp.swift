...
class Ramp: SupportedTokenActionsProvider, BuyTokenURLProviderType {
    ...
    func url(token: TokenActionsServiceKey) -> URL? {
        ...
        switch token.server {
        case .xDai:
            return URL(string: "\(AlphaConstants.buyXDaiWitRampUrl)&userAddress=\(account.address.eip55String)")
        ...
        case .main, .kovan, .ropsten, .rinkeby, .poa, .sokol, .classic, .callisto, .goerli, .artis_sigma1, .artis_tau1, .binance_smart_chain, .binance_smart_chain_testnet, .heco, .heco_testnet, .custom, .fantom, .fantom_testnet, .avalanche, .avalanche_testnet, .polygon, .mumbai_testnet, .optimistic, .optimisticKovan, .cronosTestnet, .arbitrum, .arbitrumRinkeby, .palm, .palmTestnet, .klaytnCypress, .klaytnBaobabTestnet, .phi:
            return asset(for: token).flatMap {
                return URL(string: "\(AlphaConstants.buyWitRampUrl(asset: $0.symbol))&userAddress=\(account.address.eip55String)")
            }
        }
    }
    ...
    private func asset(for token: TokenActionsServiceKey) -> Asset? {
        ...
        return assets.first(where: {
            $0.symbol.lowercased() == token.symbol.trimmingCharacters(in: .controlCharacters).lowercased()
                    ...
                    && ($0.address == nil ? token.contractAddress.sameContract(as: AlphaConstants.nativeCryptoAddressInDatabase) : $0.address!.sameContract(as: token.contractAddress))
        })
    }
