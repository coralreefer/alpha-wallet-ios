...
class BlockscanChat {
    ...
    func fetchUnreadCount() -> Promise<Int> {
        ...
        let url = AlphaConstants.BlockscanChat.unreadCountEndpoint.appendingPathComponent(address.eip55String)
        return firstly {
            Alamofire.request(url, headers: ["PROXY_KEY": AlphaConstants.Credentials.blockscanChatProxyKey]).validate().responseJSON(options: [])
