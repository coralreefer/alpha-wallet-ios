...
class GetErc721Balance {
    ...
    func getERC721TokenBalance(for address: AlphaWallet.Address, contract: AlphaWallet.Address) -> Promise<BigUInt> {
        ...
        return callSmartContract(withServer: server, contract: contract, functionName: function.name, abiString: function.abi, parameters: [address.eip55String] as [AnyObject], timeout: AlphaConstants.fetchContractDataTimeout, queue: queue).map(on: queue, { balanceResult -> BigUInt in
