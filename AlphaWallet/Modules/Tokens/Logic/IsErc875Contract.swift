...
class IsErc875Contract {
    ...
    func getIsERC875Contract(for contract: AlphaWallet.Address) -> Promise<Bool> {
        ...
        return callSmartContract(withServer: server, contract: contract, functionName: function.name, abiString: function.abi, timeout: AlphaConstants.fetchContractDataTimeout).map { dictionary -> Bool in
