...
class GetContractDecimals {
    ...
    func getDecimals(for contract: AlphaWallet.Address) -> Promise<UInt8> {
        ...
        return callSmartContract(withServer: server, contract: contract, functionName: functionName, abiString: web3swift.Web3.Utils.erc20ABI, timeout: AlphaConstants.fetchContractDataTimeout).map { dictionary -> UInt8 in
