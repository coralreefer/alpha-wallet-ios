...
class GetErc721ForTicketsBalance {
    ...
    func getERC721ForTicketsTokenBalance(for address: AlphaWallet.Address, contract: AlphaWallet.Address) -> Promise<[String]> {
        ...
        return callSmartContract(withServer: server, contract: contract, functionName: function.name, abiString: function.abi, parameters: [address.eip55String] as [AnyObject], timeout: AlphaConstants.fetchContractDataTimeout, queue: queue).map(on: queue, { balanceResult in
