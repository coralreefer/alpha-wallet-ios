...
class GetErc875Balance {
    ...
    func getERC875TokenBalance(for address: AlphaWallet.Address, contract: AlphaWallet.Address) -> Promise<[String]> {
        ...
        return callSmartContract(withServer: server, contract: contract, functionName: function.name, abiString: function.abi, parameters: [address.eip55String] as [AnyObject], timeout: AlphaConstants.fetchContractDataTimeout).map(on: queue, { balanceResult -> [String] in
