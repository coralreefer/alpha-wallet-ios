...
class IsInterfaceSupported165 {
    ...
    func getInterfaceSupported165(hash: String, contract: AlphaWallet.Address) -> Promise<Bool> {
        ...
        return firstly {
            callSmartContract(withServer: server, contract: contract, functionName: function.name, abiString: function.abi, parameters: [hash] as [AnyObject], timeout: AlphaConstants.fetchContractDataTimeout)
        }.map { result in
