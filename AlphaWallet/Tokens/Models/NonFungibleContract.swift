...
class NonFungibleContract {
    ...
    private func getTokenUriImpl(for tokenId: String, contract: AlphaWallet.Address) -> Promise<URL> {
        ...
        return firstly {
            callSmartContract(withServer: server, contract: contract, functionName: function.name, abiString: function.abi, parameters: [tokenId] as [AnyObject], timeout: AlphaConstants.fetchContractDataTimeout, queue: queue)
